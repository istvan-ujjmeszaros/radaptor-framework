...
class EmailQueueStorage
{
	...
	{
		$result = DbHelper::insertHelper(self::TABLE_TRANSACTIONAL, [
			'job_id' => $job->jobId,
	...
	public static function reserveNext(): ?array
	{
		...
		for ($attempt = 0; $attempt < 4; ++$attempt) {
			$query = "SELECT job_id
				...
				ORDER BY run_after_utc ASC, job_id ASC
				LIMIT 1";
			...
			$fetch_stmt = $pdo->prepare("SELECT * FROM `" . self::TABLE_TRANSACTIONAL . "` WHERE job_id = :job_id LIMIT 1");
	...
	{
		$row = DbHelper::selectOne(self::TABLE_TRANSACTIONAL, ['job_id' => $job_id]);
		...
		DbHelper::deleteHelper(self::TABLE_TRANSACTIONAL, ['job_id' => $job_id]);
	}

	public static function fail(string $job_id, string $error_code, string $error_message, bool $retryable): string
	{
		$row = DbHelper::selectOne(self::TABLE_TRANSACTIONAL, ['job_id' => $job_id]);
